...
public class RelationController {
    ...
    public ServerResponse<String> confirmfriend(String applicant,Byte status, HttpSession session) {
        User currentUser = (User) session.getAttribute(Const.CURRENT_USER);
        if (currentUser == null){
            return ServerResponse.createByErrorMessage("用户未登录");
        }

        return iRelationService.confirmfriend(applicant,currentUser.getUserName(),status);
    }
